    ...
    <div id="P_MS<?= $arResult['ID']; ?>" class="master-slider-parent ms-parent-id-62">
        ...
        <div id="MS<?= $arResult['ID'] ?>" class="master-slider ms-skin-light-3">
            <? foreach ($arResult["ITEMS"] as $arItem): ?>
                <div class="ms-slide" data-delay="<?= $arParams['TIME_SLIDE'] ?>"
                     <? if (!empty($arItem['CODE'])): ?>onclick="javascript:location.href='<?= $arItem['CODE'] ?>'" <? endif; ?>
                     data-fill-mode="fill">
                    <? if (strlen($arItem["PREVIEW_TEXT"]) > 1) { ?>
                        <div class="ms-slider-text-box"
                             style="right: <?= $arParams['RIGHT_PX_TEXT'] ?>px;top:<?= $arParams['TOP_PX_TEXT'] ?>px"><?= trim(strip_tags($arItem["PREVIEW_TEXT"])); ?></div>
                    <? } ?>

                    <img src="<?= $com_path ?>front/css/blank.gif" alt="" title=""
...
<script>
    ...
    masterslider_<?=$arResult['ID']?>.control('bullets', {
        autohide: false,
        overVideo: true,
        dir: 'h',
        align: "<?=$arParams['BTN_SLIDE_CONTROL']?>",
        space: 5,
        margin: 10
    });
    masterslider_<?=$arResult['ID']?>.control('timebar', {
        autohide: false,
        overVideo: true,
        align: '<?=$arParams['PROGRESS_BAR_PLACE']?>',
        color: '#<?=$arParams['PROGRESS_BAR_COLOR']?>',
        width:<?=$arParams['PROGRESS_BAR_HEIGHT']?> });
